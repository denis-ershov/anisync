...
## 2026-09-25 (fix: anime schedule sorting by air time, next week releases in today section & tomorrow releases showing today)

**Файлы:**
- `apps/web/src/components/schedule-view.tsx`
- `apps/web/src/lib/integrations/schedule-day.ts`
- `apps/web/src/lib/timezone.ts`
- `apps/web/tests/schedule-day.test.ts`
- `apps/web/scripts/verify-schedule-today.mjs`

**Изменения:**
1. **Сортировка аниме в расписании по времени выхода серии:**
   - Реализованы функции `compareScheduleAnimeByAirTime` и `sortScheduleAnimeByAirTime` в [`schedule-day.ts`](file:///e:/DEV/Project/anisync/apps/web/src/lib/integrations/schedule-day.ts), сортирующие карточки аниме внутри каждого дня недели по возрастанию времени эфира (ранние серии первыми, например 11:30 перед 16:30). При одинаковом времени выхода серии применяется вторичная сортировка по алфавиту названий на русском языке.
   - Сортировка подключена в [`schedule-view.tsx`](file:///e:/DEV/Project/anisync/apps/web/src/components/schedule-view.tsx) для списков серий каждого дня недели (`getWeekSchedule`) и секции «В процессе» (`getCatchingUpAnime`).
2. **Устранение ложного попадания аниме следующей недели в карточку «Сегодня»:**
   - В функции `getLatestAiredInstant` удалена спекулятивная эвристика с вычитанием 7 и 14 дней (`previous = next - 7d`), которая ошибочно считала любое аниме, выходящее ровно через неделю в тот же день недели, «вышедшим сегодня». Из-за этого такие тайтлы попадали в блок «Сегодня» с бейджем «через 7 дн.».
   - Теперь `getLatestAiredInstant` возвращает дату эфира только в том случае, если она уже фактически наступила (`next.getTime() <= now.getTime()`). Для будущих дат прошедший эфир не синтезируется.
   - Проверка `isRecentlyAiredForToday` теперь срабатывает строго для серий, эфир которых уже состоялся в текущие календарные сутки в таймзоне пользователя.
3. **Исправление ошибочного статуса «сегодня HH:mm» для завтрашних серий:**
   - В функции `formatNextEpisodeShort` (`apps/web/src/lib/timezone.ts`) условие `if (sameDay || hours < 24)` ошибочно помечало любые релизы в пределах 24 часов как `сегодня ${time}`, даже если календарно дата эфира приходится на завтра (например, серия завтра в 16:30 отображалась как «След. сегодня 16:30»).
   - Логика переведена на точное сопоставление календарных дней в таймзоне пользователя (`diffDateKeys`):
     - `daysDiff <= 0`: «сегодня HH:mm» (при разнице > 6 часов);
     - `daysDiff === 1`: «завтра HH:mm» (при разнице > 6 часов);
     - `hours <= 6`: «через X ч» / «через X мин» для ближайших релизов;
     - `daysDiff >= 2 && daysDiff <= 7`: «через N дн.».
4. **Корректное распределение по блокам интерфейса:**
   - Тайтлы со следующей серией через 7 дней теперь не попадают ни в день 0 («Сегодня»), ни в дни 1..6 текущей недели, а корректно отображаются в секции «В процессе» (`belongsToCatchingUp = true`), пока не наступит неделя их эфира.
5. **Тестирование:**
   - Добавлены модульные тесты для `sortScheduleAnimeByAirTime` (проверка порядка утренних/дневных/вечерних серий, обработка отсутствующих дат, алфавитный tie-break).
   - В `apps/web/tests/schedule-day.test.ts` устаревший тест заменен на проверку корректного поведения: эпизод через 7 дней не попадает в «Сегодня» и уходит в «В процессе».
   - Добавлены проверки для `formatNextEpisodeShort` на разделение «сегодня» и «завтра», дни 6, 7 и ближайшие часы. Все 107 тестов успешно проходят.
