...
export async function GET(request: NextRequest) {
  try {
    ...
    const redirectUri = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:9002'}/ru/auth/anilist/callback`;
    ...
    // Get user info
    const userResponse = await fetch(INTEGRATION_CONFIGS.anilist.apiUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (!userData) {
      console.error('No user data in response:', userResult);
      return NextResponse.redirect(`${baseUrl}/ru/settings/integrations?error=user_info_failed`);
    }
